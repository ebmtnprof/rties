...
The function returns a list of the full model results and by default produces plots of profile membership against the system variable(s), but these can be turned off by setting printPlots=F. The results below show weak evidence for empathy predicting profile membership, such that higher empathy for either partner predicts more likeilood of being in Profile 2, although neither effect is large enough to be significant. Thus, taken together, the results from this example provide some evidence that the dynamics represented by the Inertia-Coordination model are associated with empathy, such that higher-amplitude in-phase emotional oscillations are associated with more empathy.
